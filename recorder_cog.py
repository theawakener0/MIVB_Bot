...
الملخص (باللغة العربية، من 3 إلى 5 جمل كحد أقصى):"""
        ...
        summary = await chain.arun(transcript=text)
...
class RecordingView(discord.ui.View):
    ...
    async def recording_timeout(self):
        ...
        if self.voice_client and hasattr(self.voice_client, '_recording') and self.voice_client._recording:
            ...
            await self.stop_recording_logic(self.initial_interaction)
    ...
    async def post_process_recording(self, sink: WavFileSink):
        ...
        if not audio_filepath or not os.path.exists(audio_filepath):
            ...
            try:
                 ...
                 logger.error("Could not send error followup message.")
            return
...
class RecorderCog(commands.Cog):
    ...
    @app_commands.command(name="record_controls", description="Shows controls to record the current voice channel.")
    ...
    async def record_slash(self, interaction: discord.Interaction):
        ...
        view = RecordingView(self, interaction)
        self.active_views[interaction.guild_id] = view
